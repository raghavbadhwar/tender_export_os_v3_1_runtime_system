# Pricing + Compliance Gate Check — RUN-20260630130713
...
- `GOV-20260630-001`: wait for owner decision on `APR-002` and a second quote proof before pricing.
...
- `EXP-20260630-002`: not in this pricing run because status is `APPROVAL_REQUIRED`; note the audit warning that only one quote proof is present in `quote_master.csv` despite `pricing_done=TRUE`, and buyer/source identity is still missing.
...
- `AGENTS.md` pricing/compliance rules
- `skills/pricing-proof/SKILL.md`
