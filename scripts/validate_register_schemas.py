...
def schema_data_path(schema: dict[str, Any], public_template: bool = False) -> Path:
    rel_file = schema.get("file", "")
    if not public_template:
        return PROJECT_ROOT / rel_file
    if rel_file == "data/events.jsonl":
        return PROJECT_ROOT / "data" / "examples" / "events.example.jsonl"
    if rel_file.startswith("data/") and rel_file.endswith(".csv"):
        stem = Path(rel_file).stem
        return PROJECT_ROOT / "data" / "examples" / f"{stem}.example.csv"
    return PROJECT_ROOT / rel_file
...
def validate_csv_schema(schema_path: Path, public_template: bool = False) -> list[str]:
    errors: list[str] = []
    schema = load_json(schema_path)
    if "file" not in schema:
        return []
    csv_path = schema_data_path(schema, public_template)
    ...
    if missing_columns:
        errors.append(f"{label}: missing columns {missing_columns}")

    primary_key = schema.get("primary_key")
    seen_keys: set[str] = set()
    for index, row in enumerate(rows, start=2):
        if None in row:
            errors.append(f"{label}:{index}: extra cells {row[None]}")
        if primary_key:
            value = row.get(primary_key, "")
            if not value:
                errors.append(f"{label}:{index}: missing primary key {primary_key}")
            elif value in seen_keys:
                errors.append(f"{label}:{index}: duplicate primary key {value}")
            seen_keys.add(value)
        ...
        for field in schema.get("date_fields", []):
            ...
            if value and not parse_date(value):
                errors.append(f"{label}:{index}: {field}={value!r} is not an ISO date")

        for field in schema.get("datetime_fields", []):
            value = row.get(field, "")
            if value and not parse_datetime(value):
                errors.append(f"{label}:{index}: {field}={value!r} is not an ISO datetime")

        for field in schema.get("numeric_fields", []):
            value = row.get(field, "")
            if value and not parse_number(value):
                errors.append(f"{label}:{index}: {field}={value!r} is not numeric")

        for field in schema.get("url_fields", []):
            value = row.get(field, "")
            if value and not URL_RE.match(value):
                errors.append(f"{label}:{index}: {field}={value!r} is not a URL")
        ...
        for rule in schema.get("cross_field_rules", []):
            ...
            if all(row.get(field, "") == value for field, value in expected.items()):
                ...
                if requires_any and not any(row.get(field, "") for field in requires_any):
                    errors.append(f"{label}:{index}: {rule.get('error', rule.get('rule', 'cross-field rule failed'))}")
...
def validate_events(public_template: bool = False) -> list[str]:
    schema = load_json(EVENT_SCHEMA)
    path = schema_data_path(schema, public_template)
        ...
        for index, line in enumerate(f, start=1):
            ...
            for field in schema["required_fields"]:
                if field not in event:
                    errors.append(f"{label}:{index}: missing field {field}")
            event_id = event.get("event_id", "")
            if not event_id:
                errors.append(f"{label}:{index}: missing event_id")
            elif event_id in seen:
                errors.append(f"{label}:{index}: duplicate event_id {event_id}")
            seen.add(event_id)
            if event.get("event_type") not in schema["event_types"]:
                errors.append(f"{label}:{index}: unknown event_type {event.get('event_type')!r}")
            if event.get("object_type") not in schema["object_types"]:
                errors.append(f"{label}:{index}: unknown object_type {event.get('object_type')!r}")
            if not parse_datetime(str(event.get("event_time", ""))):
                errors.append(f"{label}:{index}: event_time is not ISO datetime")
...
def main() -> int:
    ...
    parser.add_argument("--public-template", action="store_true", help="Validate sanitized data/examples files")
    args = parser.parse_args()

    schema_paths = sorted(path for path in SCHEMA_DIR.glob("*.schema.json") if path.name != "event.schema.json")
    errors: list[str] = []
    for schema_path in schema_paths:
        errors.extend(validate_csv_schema(schema_path, public_template=args.public_template))
    if not args.csv_only:
        errors.extend(validate_events(public_template=args.public_template))
