# Weekly Learning Council Packet

`scripts/build_weekly_learning_packet.py` assembles the Hermes weekly learning packet for `prompts/hermes/weekly_learning_council.md`.

The packet includes:

- outcomes;
- owner correction availability;
- failed runs;
- source yield;
- supplier performance;
- buyer reply results;
- forecast error/calibration state;
- policy denials;
- unresolved contradictions;
- skill/profile usage;
- previous proposal effectiveness.

This is an internal learning artifact only. It does not apply proposals, change source weights, promote suppliers, train/promote models, send messages, submit tenders, or make financial/legal/compliance commitments.
